/** Sort lessons the same way on dashboard, player playlist, and server merge. */
...
export const getSubjectVideosForPlayAll = (contents = [], chapters = []) =>
  sortSubjectContents(
    (contents || []).filter((row) => row.type === "video"),
    chapters
  );
